...
export interface DrugItem {
  ...
  pediatricDoseUnit?: 'mg/kg' | 'ml/kg' | 'g/kg' | 'mg/m2' | 'IU/kg' | 'fixed';
...
export const CLINICAL_SYMPTOM_MAP: Record<string, string[]> = {
  ...
  bites: ['arv', 'rabies', 'erig', 'hrig', 'tetanus', 'asv', 'anti-snake', 'snake bite', 'dog bite', 'antivenom'],
  rabies: ['arv', 'rabies', 'erig', 'hrig', 'tetanus', 'rabipur', 'abhayrab'],
  endocrine: ['metformin', 'glimepiride', 'gliclazide', 'vildagliptin', 'teneligliptin', 'sitagliptin', 'dapagliflozin', 'empagliflozin', 'voglibose', 'pioglitazone', 'semaglutide', 'tirzepatide', 'insulin', 'lantus', 'levothyroxine', 'thyronorm', 'eltroxin', 'carbimazole', 'saroglitazar'],
  general: ['paracetamol', 'pan 40', 'pantoprazole', 'amoxicillin', 'augmentin', 'azithromycin', 'cefixime', 'dextromethorphan', 'cetirizine', 'ors', 'vitamin c', 'multivitamin', 'calcium', 'doxycycline', 'ciprofloxacin', 'ranitidine', 'famotidine', 'gelusil', 'cremaffin', 'dulcolax', 'fybogel', 'bilastine', 'allegra', 'loratadine', 'avil', 'benadryl', 'ambroxol', 'nac', 'acebrophylline'],
  infectious: ['amoxicillin', 'augmentin', 'azithromycin', 'cefixime', 'ceftriaxone', 'ciprofloxacin', 'levofloxacin', 'metronidazole', 'doxycycline', 'clarithromycin', 'meropenem', 'vancomycin', 'piperacillin', 'clindamycin', 'fluconazole', 'itraconazole', 'acyclovir', 'oseltamivir', 'colistin', 'linezolid'],
  toxicology: ['atropine', 'pralidoxime', 'pam', 'asv', 'anti-snake', 'activated charcoal', 'n-acetylcysteine', 'mucinac', 'vitamin k1', 'phytomenadione', 'thiamine', 'flumazenil', 'naloxone', 'calcium gluconate', 'sodium bicarbonate', 'magnesium sulfate', 'defuroxamine', 'penicillamine'],
  respiratory: ['foracort', 'seretide', 'duolin', 'levosalbutamol', 'budesonide', 'montelukast', 'acebrophylline', 'doxofylline', 'cough syrup', 'ambroxol', 'salbutamol', 'fluticasone', 'ipratropium'],
  trauma: ['paracetamol', 'tramadol', 'tetanus', 'wound dressing', 'povidone', 'betadine', 'silver sulfadiazine', 'burnol', 'diclofenac', 'aceclofenac', 'normal saline', 'iv fluids'],
  pediatric: ['pediatric', 'syrup', 'drops', 'dry syrup', 'suspension', 'rotavirus', 'bcg', 'bopv', 'pentavalent', 'hexavalent', 'ipv', 'pcv', 'mr', 'mmr', 'je', 'tcv', 'vitamin a', 'paracetamol drops', 'azithromycin syrup', 'cefixime syrup', 'moxkind-cv'],
...
export function searchClinicalDrugs(
  ...
  const filtered = catalog.filter((rawDrug) => {
    ...
    // Direct Specialty Key Match (e.g. 'bites', 'cardio', 'dermatology', 'endocrine', 'ent', 'emergency', 'gastro', 'general', 'gynae', 'infectious', 'nephrology', 'neurology', 'ophthalmology', 'ortho', 'pediatric', 'psychiatry', 'respiratory', 'toxicology', 'trauma')
...
export function calculatePediatricDose(
  ...
  return [
    ...
    {
      drugName: 'Ambroxol + Terbutaline + Guaifenesin Syrup (Ascoril LS / Mucolite / Ambrolite)',
      recommendedDoseMg: Math.round(weightKg * 0.3 * 10),
      formulation: 'Syrup (15mg + 1.25mg + 50mg / 5ml)',
      calculatedVolumeMl: `${Math.min(5, Math.max(1.25, parseFloat((weightKg * 0.3).toFixed(1))))} ml`,
      frequency: '3 times daily after food (1-1-1 for 5 days)',
      notes: `Target: 0.3 ml/kg/dose (${Math.min(5, Math.max(1.25, parseFloat((weightKg * 0.3).toFixed(1))))} ml per dose)`,
    },
    {
      drugName: 'Levosalbutamol + Ambroxol + Guaifenesin Syrup',
      recommendedDoseMg: Math.round(weightKg * 0.25 * 10),
      formulation: 'Syrup (0.63mg + 15mg + 50mg / 5ml)',
      calculatedVolumeMl: `${Math.min(5, Math.max(1.25, parseFloat((weightKg * 0.25).toFixed(1))))} ml`,
      frequency: '3 times daily after food (1-1-1 for 5 days)',
      notes: `Target: 0.25 ml/kg/dose (${Math.min(5, Math.max(1.25, parseFloat((weightKg * 0.25).toFixed(1))))} ml per dose)`,
    },
    {
...
export const COMPREHENSIVE_GENERIC_DRUGS: DrugItem[] = [
  ...
  { id: 'gen_cefix100', genericName: 'Cefixime 100mg Dispersible Tablet', category: 'all', dosage: '1 tablet dissolved in 15ml water twice daily', duration: '5 days', keywords: 'cefixime zifi dt pediatric antibiotic fever urti', pediatricDoseType: 'per_kg', pediatricDoseValue: 4, pediatricDoseUnit: 'mg/kg', pediatricDoseRuleText: '4 mg/kg per dose BD = 8 mg/kg/day' },
  { id: 'gen_cefix_syp', genericName: 'Cefixime 50mg/5ml Dry Syrup', category: 'pediatric', dosage: '5ml twice daily (1-0-1)', duration: '5 days', keywords: 'cefixime zifi syrup pediatric antibiotic fever', maxAge: 12, pediatricDoseType: 'per_kg', pediatricDoseValue: 4, pediatricDoseUnit: 'mg/kg', pediatricConcentrationMgPerMl: 10, pediatricDoseRuleText: '4 mg/kg per dose BD (0.4 ml/kg BD)' },
  ...
  { id: 'gen_azee250', genericName: 'Azithromycin 250mg Tablet', category: 'all', dosage: '1 tablet once daily', duration: '5 days', keywords: 'azithromycin azee azithral macrolide antibiotic urti', pediatricDoseType: 'per_kg', pediatricDoseValue: 10, pediatricDoseUnit: 'mg/kg', pediatricDoseRuleText: '10 mg/kg once daily for 3 days' },
  { id: 'gen_azee_syp', genericName: 'Azithromycin 100mg/5ml Oral Suspension', category: 'pediatric', dosage: '5ml once daily for 3 days', duration: '3 days', keywords: 'azithromycin azee liquid syrup pediatric macrolide antibiotic', maxAge: 12, pediatricDoseType: 'per_kg', pediatricDoseValue: 10, pediatricDoseUnit: 'mg/kg', pediatricConcentrationMgPerMl: 20, pediatricDoseRuleText: '10 mg/kg once daily (0.5 ml/kg OD)' },
  
  // LAXATIVES & STOOL SOFTENERS (SAFE PEDIATRIC VS CONTRAINDICATED)
  { id: 'gen_lactulose_syp', genericName: 'Lactulose 3.33g/5ml Syrup (Duphalac / Loose / Lactecon)', category: 'all', dosage: '0.5 ml/kg/day (5ml to 10ml once daily at bedtime)', duration: '7 days', keywords: 'lactulose duphalac loose constipation laxative stool softener pediatric constipation', pediatricDoseType: 'per_kg', pediatricDoseValue: 0.5, pediatricDoseUnit: 'ml/kg', pediatricDoseRuleText: 'Target: 0.5 ml/kg once daily at bedtime (Max 15ml). Safe first-line osmotic laxative for infants and children.' },
  { id: 'gen_peg3350_sachet', genericName: 'Polyethylene Glycol 3350 Powder Sachet (PEG 3350 / Pegmove / Cremaffin-Fresh)', category: 'all', dosage: '0.5 g/kg/day dissolved in water once daily in morning', duration: '14 days', keywords: 'peg 3350 polyethylene glycol pegmove cremaffin fresh sachet constipation laxative stool softener pediatric chronic constipation', pediatricDoseType: 'per_kg', pediatricDoseValue: 0.5, pediatricDoseUnit: 'g/kg', pediatricDoseRuleText: 'Target: 0.5 g/kg/day in water (Gold standard ESPGHAN pediatric chronic constipation therapy).' },
  { id: 'gen_cremaffin_syp', genericName: 'Liquid Paraffin + Milk of Magnesia Syrup (Cremaffin)', category: 'adult', dosage: '15ml to 30ml at bedtime with glass of water', duration: '3 days', keywords: 'cremaffin liquid paraffin milk of magnesia constipation adult laxative', minAge: 12, minWeight: 35, pediatricDoseType: 'fixed_flat', pediatricDoseRuleText: '⚠️ CONTRAINDICATED in Children < 3 years (Aspiration Lipid Pneumonia Risk!). Use Lactulose 5ml OD or PEG 3350 for pediatric constipation.' },
  ...
  { id: 'gen_clari250', genericName: 'Clarithromycin 250mg Tablet (Claribid 250)', category: 'all', dosage: '1 tablet twice daily', duration: '7 days', keywords: 'clarithromycin claribid 250 macrolide antibiotic RTI', pediatricDoseType: 'per_kg', pediatricDoseValue: 7.5, pediatricDoseUnit: 'mg/kg', pediatricDoseRuleText: '7.5 mg/kg per dose BD = 15 mg/kg/day' },
  ...
  { id: 'gen_xylomet005', genericName: 'Xylometazoline 0.05% Pediatric Nasal Drops (Otrivin Pediatric)', category: 'pediatric', dosage: '1 to 2 drops in each nostril twice daily (max 5 days)', duration: '3 days', keywords: 'xylometazoline otrivin pediatric nasal drops blocked nose decongestant', maxAge: 12, pediatricDoseType: 'age_tiered', pediatricDoseRuleText: '1 to 2 drops in each nostril twice daily (Max 5 days)' },
  ...
  { id: 'gen_oxymet0025', genericName: 'Oxymetazoline 0.025% Pediatric Nasal Drops (Nasivion Pediatric / Oxy)', category: 'pediatric', dosage: '1 to 2 drops in each nostril twice daily', duration: '3 days', keywords: 'oxymetazoline nasivion oxy pediatric nasal drops blocked nose decongestant', pediatricDoseType: 'age_tiered', pediatricDoseRuleText: '1 to 2 drops in each nostril twice daily (Max 3 days)' },
  ...
  { id: 'gen_ascoril_ls', genericName: 'Ambroxol + Terbutaline + Guaifenesin Syrup (Ascoril LS / Mucolite / Ambrolite)', category: 'pediatric', dosage: '0.3ml/kg/dose (Calculated volume per weight 3 times daily)', duration: '5 days', keywords: 'ambroxol terbutaline guaifenesin ascoril ls mucolite ambrolite syrup wet cough phlegm bronchospasm asthma pediatric', maxAge: 12 },
  { id: 'gen_levo_ambro', genericName: 'Levosalbutamol + Ambroxol + Guaifenesin Syrup', category: 'pediatric', dosage: '0.25ml/kg/dose 3 times daily', duration: '5 days', keywords: 'levosalbutamol ambroxol guaifenesin syrup pediatric cough asthma', maxAge: 12 },
  ...
  { id: 'fdc1', genericName: 'Aceclofenac + Paracetamol Tablet', category: 'adult', dosage: '100mg/325mg (1-0-1 after food)', duration: '5 days', keywords: 'pain fever inflammation zerodol-p aceclofenac paracetamol tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc2', genericName: 'Aceclofenac + Paracetamol + Serratiopeptidase Tablet', category: 'adult', dosage: '100mg/325mg/15mg (1-0-1 after food)', duration: '5 days', keywords: 'zerodol-sp aceclofenac paracetamol serratiopeptidase tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc3', genericName: 'Aceclofenac + Paracetamol + Chlorzoxazone Tablet', category: 'adult', dosage: '100mg/325mg/250mg (1-0-1 after food)', duration: '5 days', keywords: 'zerodol-mr aceclofenac paracetamol chlorzoxazone tablet', minAge: 18, minWeight: 40 },
  { id: 'fdc4', genericName: 'Diclofenac Potassium + Paracetamol Tablet', category: 'adult', dosage: '50mg/325mg (1-0-1 after food)', duration: '3 days', keywords: 'voveran-p diclofenac paracetamol tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc5', genericName: 'Diclofenac + Serratiopeptidase Tablet', category: 'adult', dosage: '50mg/10mg (1-0-1 after food)', duration: '5 days', keywords: 'voveran-s diclofenac serratiopeptidase tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc6', genericName: 'Ibuprofen + Paracetamol Tablet', category: 'adult', dosage: '400mg/325mg (1-0-1 after food)', duration: '3 days', keywords: 'combiflam ibuprofen paracetamol tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc7', genericName: 'Mefenamic Acid + Paracetamol Tablet', category: 'adult', dosage: '500mg/325mg (1-0-1 after food)', duration: '3 days', keywords: 'meftal-p mefenamic paracetamol tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc8', genericName: 'Mefenamic Acid + Paracetamol Syrup (100mg/125mg per 5ml)', category: 'pediatric', dosage: '5ml (b.d. after food)', duration: '3 days', keywords: 'meftal-p syrup pediatric mefenamic paracetamol', minAge: 2, maxAge: 12 },
  { id: 'fdc9', genericName: 'Tramadol + Paracetamol Tablet', category: 'adult', dosage: '37.5mg/325mg (1-0-1 S.O.S)', duration: '3 days', keywords: 'ultracet tramadol paracetamol tablet', minAge: 18, minWeight: 40 },
  { id: 'fdc10', genericName: 'Etoricoxib + Thiocolchicoside Tablet', category: 'adult', dosage: '60mg/4mg (1-0-1 after food)', duration: '5 days', keywords: 'nucoxia-mr etoricoxib thiocolchicoside tablet', minAge: 18, minWeight: 40 },
  { id: 'fdc11', genericName: 'Trypsin + Chymotrypsin Tablet', category: 'adult', dosage: '100,000 Armour units (1-1-1 30m before food)', duration: '5 days', keywords: 'chymoral forte trypsin chymotrypsin tablet', minAge: 12 },
  { id: 'fdc12', genericName: 'Pantoprazole + Domperidone SR Capsule', category: 'adult', dosage: '40mg/30mg SR (1-0-0 before food)', duration: '7 days', keywords: 'pan-d pantocid-d pantoprazole domperidone capsule', minAge: 18, minWeight: 40 },
  { id: 'fdc13', genericName: 'Pantoprazole + Levosulpiride SR Capsule', category: 'adult', dosage: '40mg/75mg SR (1-0-0 before food)', duration: '14 days', keywords: 'pan-ls pantoprazole levosulpiride capsule', minAge: 18 },
  { id: 'fdc14', genericName: 'Rabeprazole + Domperidone SR Capsule', category: 'adult', dosage: '20mg/30mg SR (1-0-0 before food)', duration: '7 days', keywords: 'rabeloc-d rabekind-d rabeprazole domperidone capsule', minAge: 18, minWeight: 40 },
  { id: 'fdc15', genericName: 'Rabeprazole + Levosulpiride SR Capsule', category: 'adult', dosage: '20mg/75mg SR (1-0-0 before food)', duration: '14 days', keywords: 'rabeloc-ls rabeprazole levosulpiride capsule', minAge: 18 },
  { id: 'fdc16', genericName: 'Omeprazole + Domperidone Capsule', category: 'adult', dosage: '20mg/10mg (1-0-0 before food)', duration: '7 days', keywords: 'omez-d omeprazole domperidone capsule', minAge: 12 },
  { id: 'fdc17', genericName: 'Esomeprazole + Domperidone SR Capsule', category: 'adult', dosage: '40mg/30mg SR (1-0-0 before food)', duration: '7 days', keywords: 'esomeprazole domperidone capsule', minAge: 18, minWeight: 40 },
  { id: 'fdc18', genericName: 'Dicyclomine + Paracetamol Tablet', category: 'adult', dosage: '20mg/500mg (1-0-1 S.O.S for abdominal spasms)', duration: '3 days', keywords: 'cyclopam meftal-spas dicyclomine paracetamol tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc19', genericName: 'Drotaverine + Mefenamic Acid Tablet', category: 'adult', dosage: '80mg/250mg (1-0-1 after food)', duration: '3 days', keywords: 'drotin-m drotaverine mefenamic tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc20', genericName: 'Amoxicillin + Clavulanic Acid Tablet 625mg', category: 'adult', dosage: '500mg/125mg (1-0-1 after food)', duration: '5 days', keywords: 'moxkind-cv augmentin 625 amoxicillin clavulanate tablet', minAge: 12, minWeight: 40 },
  { id: 'fdc21', genericName: 'Amoxicillin + Clavulanate Dry Syrup (228mg/5ml)', category: 'pediatric', dosage: '5ml (b.d.)', duration: '5 days', keywords: 'moxkind-cv augmentin dry syrup amoxicillin clavulanate', minAge: 1, maxAge: 12 },
  { id: 'fdc22', genericName: 'Cefixime + Ofloxacin Tablet', category: 'adult', dosage: '200mg/200mg (1-0-1 after food)', duration: '5 days', keywords: 'cefixime ofloxacin tablet', minAge: 18, minWeight: 40 },
  { id: 'fdc23', genericName: 'Ofloxacin + Ornidazole Tablet', category: 'adult', dosage: '200mg/500mg (1-0-1 after food)', duration: '5 days', keywords: 'oflox-oz ofloxacin ornidazole tablet', minAge: 18, minWeight: 40 },
  { id: 'fdc24', genericName: 'Ofloxacin + Ornidazole Syrup (50mg/125mg per 5ml)', category: 'pediatric', dosage: '5ml (b.d.)', duration: '5 days', keywords: 'oflox-oz syrup pediatric ofloxacin ornidazole', minAge: 2, maxAge: 12 },
  { id: 'fdc25', genericName: 'Ciprofloxacin + Tinidazole Tablet', category: 'adult', dosage: '500mg/600mg (1-0-1 after food)', duration: '5 days', keywords: 'cipro-tz ciprofloxacin tinidazole tablet', minAge: 18, minWeight: 40 },
  { id: 'fdc26', genericName: 'Inj Piperacillin + Tazobactam IV Vial (Zosyn 4.5g)', category: 'adult', dosage: '4g/0.5g IV infusion over 30 mins 6-8 hourly', duration: '7 days', keywords: 'inj zosyn piperacillin tazobactam iv injection', minAge: 18 },
  { id: 'fdc27', genericName: 'Inj Cefoperazone + Sulbactam IV Vial (Magnex 1.5g)', category: 'adult', dosage: '1g/0.5g IV every 12 hours', duration: '7 days', keywords: 'inj magnex cefoperazone sulbactam iv injection', minAge: 12 },
  { id: 'fdc28', genericName: 'Inj Ceftriaxone + Sulbactam IV Vial (1.5g)', category: 'adult', dosage: '1g/0.5g IV once daily', duration: '7 days', keywords: 'inj ceftriaxone sulbactam iv injection', minAge: 12 },
  { id: 'fdc29', genericName: 'Inj Imipenem + Cilastatin IV Vial', category: 'adult', dosage: '500mg/500mg IV 6 hourly', duration: '7 days', keywords: 'inj imipenem cilastatin iv injection', minAge: 18 },
  { id: 'fdc30', genericName: 'Levocetirizine + Montelukast Tablet', category: 'adult', dosage: '5mg/10mg (0-0-1 at night)', duration: '10 days', keywords: 'monticope levocetirizine montelukast tablet', minAge: 15, minWeight: 40 },
  { id: 'fdc31', genericName: 'Levocetirizine + Montelukast Syrup', category: 'pediatric', dosage: '5ml (at night)', duration: '7 days', keywords: 'monticope syrup pediatric levocetirizine montelukast', minAge: 2, maxAge: 12 },
  { id: 'fdc32', genericName: 'Fexofenadine + Montelukast Tablet', category: 'adult', dosage: '120mg/10mg (0-0-1 at night)', duration: '10 days', keywords: 'fexofenadine montelukast tablet', minAge: 15 },
  { id: 'fdc33', genericName: 'Formoterol + Budesonide Inhaler (Foracort 200)', category: 'all', dosage: '2 puffs (b.d. with mouth rinse)', duration: '30 days', keywords: 'foracort formoterol budesonide inhaler' },
  { id: 'fdc34', genericName: 'Salmeterol + Fluticasone Inhaler (Seretide 125)', category: 'all', dosage: '2 puffs (b.d.)', duration: '30 days', keywords: 'seretide salmeterol fluticasone inhaler' },
  { id: 'fdc35', genericName: 'Levosalbutamol + Ipratropium Respules (Duolin Neb)', category: 'all', dosage: '1 respule neb (t.d.s)', duration: '5 days', keywords: 'duolin levosalbutamol ipratropium respules nebulization' },
  { id: 'fdc36', genericName: 'Ambroxol + Terbutaline + Guaifenesin Expectorant Syrup', category: 'adult', dosage: '10ml (t.d.s after food)', duration: '5 days', keywords: 'ambroxol terbutaline guaifenesin expectorant syrup', minAge: 12 },
  { id: 'fdc37', genericName: 'Dextromethorphan + Chlorpheniramine + Phenylephrine Syrup', category: 'adult', dosage: '5ml to 10ml (t.d.s)', duration: '5 days', keywords: 'dextromethorphan chlorpheniramine phenylephrine syrup', minAge: 6 },
  { id: 'fdc38', genericName: 'Telmisartan + Amlodipine Tablet (Telma-AM)', category: 'adult', dosage: '40mg/5mg (1-0-0 morning)', duration: '30 days', keywords: 'telmisartan amlodipine telma-am tablet', minAge: 18, minWeight: 40 },
  ...
  { id: 'fdc39', genericName: 'Telmisartan + Hydrochlorothiazide Tablet (Telma-H)', category: 'adult', dosage: '40mg/12.5mg (1-0-0 morning)', duration: '30 days', keywords: 'telmisartan hydrochlorothiazide telma-h tablet hypertension high bp blood pressure antihypertensive', minAge: 18, minWeight: 40 },
  { id: 'fdc40', genericName: 'Amlodipine + Atenolol Tablet (Amlokind-AT / Amcard-AT)', category: 'adult', dosage: '5mg/50mg (1-0-0 morning)', duration: '30 days', keywords: 'amlodipine atenolol amcard-at tablet hypertension high bp blood pressure antihypertensive', minAge: 18, minWeight: 40 },
  { id: 'fdc41', genericName: 'Losartan + Hydrochlorothiazide Tablet (Covance-D)', category: 'adult', dosage: '50mg/12.5mg (1-0-0 morning)', duration: '30 days', keywords: 'losartan hydrochlorothiazide covance-d tablet hypertension high bp blood pressure antihypertensive', minAge: 18, minWeight: 40 },
  { id: 'fdc43', genericName: 'Aspirin + Clopidogrel Capsule (Ecosprin-AV)', category: 'adult', dosage: '75mg/75mg (0-1-0 after lunch)', duration: '30 days', keywords: 'aspirin clopidogrel ecosprin-av capsule antiplatelet CAD stroke heart cardiology', minAge: 18, minWeight: 40 },
  { id: 'fdc44', genericName: 'Rosuvastatin + Fenofibrate Tablet', category: 'adult', dosage: '10mg/160mg (0-0-1 at night)', duration: '30 days', keywords: 'rosuvastatin fenofibrate tablet lipid cholesterol triglycerides cardiology', minAge: 18 },
  { id: 'fdc45', genericName: 'Metformin + Glimepiride Tablet (Amaryl-M / Glycomet-GP)', category: 'adult', dosage: '500mg/1mg (1-0-1 before food)', duration: '30 days', keywords: 'metformin glimepiride amaryl-m glycomet-gp tablet diabetes blood sugar t2dm antidiabetic endocrinology', minAge: 18, minWeight: 40 },
  { id: 'fdc46', genericName: 'Metformin + Teneligliptin SR Tablet (Tenepure-M)', category: 'adult', dosage: '500mg/20mg (1-0-1 after food)', duration: '30 days', keywords: 'metformin teneligliptin tenepure-m zita-plus-m tablet diabetes blood sugar t2dm antidiabetic endocrinology', minAge: 18 },
  { id: 'fdc47', genericName: 'Metformin + Sitagliptin Tablet (Janumet 50/500)', category: 'adult', dosage: '500mg/50mg (1-0-1 after food)', duration: '30 days', keywords: 'metformin sitagliptin janumet tablet diabetes blood sugar t2dm antidiabetic endocrinology', minAge: 18 },
  { id: 'fdc48', genericName: 'Metformin + Dapagliflozin SR Tablet (Forxiga-M)', category: 'adult', dosage: '500mg/10mg (1-0-0 morning)', duration: '30 days', keywords: 'metformin dapagliflozin forxiga-m tablet diabetes blood sugar t2dm antidiabetic endocrinology', minAge: 18 },
  { id: 'fdc49', genericName: 'Pregabalin + Methylcobalamin Capsule (Maxgalin-M)', category: 'adult', dosage: '75mg/1500mcg (0-0-1 at night)', duration: '30 days', keywords: 'pregabalin methylcobalamin maxgalin-m capsule neuropathy nerve pain diabetic neuropathy neurology', minAge: 18 },
  { id: 'fdc50', genericName: 'Gabapentin + Methylcobalamin Tablet (Gabapin-M)', category: 'adult', dosage: '300mg/500mcg (0-0-1 at night)', duration: '30 days', keywords: 'gabapentin methylcobalamin gabapin-m tablet neuropathy nerve pain diabetic neuropathy neurology', minAge: 18 },
